// ===================== renderers/shops.js — المحلات (معدل) =====================

function addShop() {
  const name = document.getElementById('sh-name').value.trim();
  const phone = document.getElementById('sh-phone').value.trim();
  if (!name) return alertMsg('أدخل اسم المحل', 'warning');
  ...
  save();
  renderShops();
  showToast(`تم إضافة المحل ${name}`, 'success');
}
...
function delShop(id) {
  ...
  save();
  renderShops();
  showToast('تم حذف المحل', 'success');
}
...
function renderShops() {
  ...
  container.innerHTML = S.shops.map(sh => {
    const lahu = (sh.lahu || []).reduce((s, x) => s + (x.total || 0), 0);
    const alihi = (sh.alihi || []).reduce((s, x) => s + (x.total || 0), 0);
    ...
    return `<div class="card" style="cursor:pointer" onclick="openShopDetail(${sh.id})">
      <div style="padding:10px 12px;background:#f3e5f5;display:flex;align-items:center;gap:8px">
        <span>🏬</span>
        <div>
          <div style="font-weight:800;color:#8e24aa">${escapeHtml(sh.name)}</div>
          <div style="font-size:0.74rem;color:var(--gray)">${escapeHtml(sh.phone || 'لا يوجد هاتف')}</div>
        </div>
        <div style="margin-right:auto;display:flex;align-items:center;gap:8px">
          <div style="text-align:left">
            ...
            <div style="font-weight:900;color:${net >= 0 ? 'var(--green)' : 'var(--red)'}">${N(Math.abs(net))} ${net >= 0 ? 'له' : 'عليه'}</div>
          </div>
...
function openShopDetail(id) {
  ...
  document.getElementById('shd-name').textContent = escapeHtml(sh.name);

  const lahuTot = sh.lahu.reduce((s, x) => s + (x.total || 0), 0);
  const alihiTot = sh.alihi.reduce((s, x) => s + (x.total || 0), 0);
  const net = lahuTot - alihiTot;
  document.getElementById('shd-bal').textContent = `رصيد: ${N(Math.abs(net))} ${net >= 0 ? 'له' : 'عليه'}`;

  const custOpts = '<option value="">نقدي</option>' +
    S.customers.map(c => `<option value="c_${c.id}">${escapeHtml(c.name)}</option>`).join('') +
    S.employees.map(e => `<option value="e_${e.id}">👷 ${escapeHtml(e.name)}</option>`).join('') +
    S.partners.map(p => `<option value="p_${p.id}">🤝 ${escapeHtml(p.name)}</option>`).join('');

  const lahuRows = sh.lahu.map((x, xi) =>
    `<tr>
      <td style="padding:5px;font-size:0.75rem">${escapeHtml(x.date)}</td>
      <td style="padding:5px;font-weight:700">${escapeHtml(x.productName)}</td>
      ...
      <td style="padding:5px;font-size:0.74rem">${escapeHtml(x.targetName || 'نقدي')}</td>
...
function showShopTab(tab, btn, id) {
  const lahuDiv = document.getElementById(`sh-lahu-${id}`);
  const alihiDiv = document.getElementById(`sh-alihi-${id}`);
  if (lahuDiv) lahuDiv.style.display = tab === 'lahu' ? 'block' : 'none';
  if (alihiDiv) alihiDiv.style.display = tab === 'alihi' ? 'block' : 'none';
}
...
function calcShop(id) {
  ...
  const totalInput = document.getElementById(`sh-tot-${id}`);
...
function addShopLahu(id) {
  ...
  if (!pname) return alertMsg('أدخل اسم الصنف', 'warning');
  const units = wt > 0 ? wt : qty;
  if (!units || !price) return alertMsg('أدخل الكمية والسعر', 'warning');
  ...
  if (tv.startsWith('c_')) {
    ...
    if (cust) {
      ...
      cust.ledger.push({ date: S.date, type: 'order', amount: total, ref: pname, isTarhil: false });
    }
  } else if (tv.startsWith('e_')) {
    const emp = S.employees.find(e => e.id == tv.slice(2));
    if (emp) {
      targetName = '👷 ' + emp.name;
      emp.payments.push({ amount: total, note: `بضاعة: ${pname}`, date: S.date });
    }
  } else if (tv.startsWith('p_')) {
    const part = S.partners.find(p => p.id == tv.slice(2));
    if (part) {
      targetName = '🤝 ' + part.name;
      part.payments.push({ amount: total, note: `بضاعة: ${pname}`, date: S.date });
    }
  }
  ...
  ['sh-pname', 'sh-qty', 'sh-wt', 'sh-price'].forEach(pfx => {
  ...
  openShopDetail(id);
  renderAll();
  showToast(`تم إضافة ${pname} بقيمة ${N(total)} جنيه`, 'success');
}
